...
export default function AdminProfile() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => apiRequest(`/users/${user.id}`, { method: 'PUT', body: data }),
  ...
  const handleSave = () => updateMutation.mutate({ fullName, phoneNumber });
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <Button variant={editing ? 'secondary' : 'primary'} size="sm"
          onClick={() => {
            if (editing) { setFullName(user.fullName || user.full_name || ''); setPhoneNumber(user.phone_number || ''); }
            setEditing(!editing);
          }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <CardBody className="text-center">
            <div className="w-20 h-20 rounded-full bg-teal/10 flex items-center justify-center mx-auto mb-3">
              <span className="text-2xl font-bold text-teal">
                {(user.fullName || user.full_name || '?').split(' ').map((s) => s[0]).join('').toUpperCase().slice(0, 2)}
              </span>
            </div>
        ...
        <Card className="lg:col-span-2">
          <CardHeader><h3 className="text-sm font-semibold">Account Details</h3></CardHeader>
